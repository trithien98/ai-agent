...
export const runLLMWithFunctionInfo = async ({
  messages,
  tools,
}: {
  messages: AIMessage[]
  tools: Tool[]
}): Promise<{ response: string; hadFunctionCalls: boolean }> => {
  // Convert messages to Gemini format and filter out unsupported types
  const history = messages
    .map((msg) => {
      if (msg.role === 'user') {
  ...
  try {
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      return { response: finalResponse, hadFunctionCalls: true }
    }

    // No function calls, return the text response
    return { response: response.text(), hadFunctionCalls: false }
  } catch (error) {
    console.error('LLM execution error:', error)
    throw error
  }
